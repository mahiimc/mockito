...
public class PersonRepository {
	...
	public Optional<Person> findByEmailAndPassword(String email, String password) {
		return PERSON_DB.values().stream()
				.filter(p -> p.getEmail().equalsIgnoreCase(email) && p.getPassword().equals(password)).findAny();
	}
	...
	public Optional<Person> findByEmail(String email) {
		return PERSON_DB.values().stream().filter(p -> p.getEmail().equalsIgnoreCase(email)).findAny();
	}
